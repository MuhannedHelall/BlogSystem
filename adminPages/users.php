...
if ($_SESSION['user']['role_id'] == '1') {
    ...
    $id = 0;
    try {
        $stmt = $conn->prepare("SELECT * FROM users WHERE role_id = '2' AND visibility = '1';");
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $users = $stmt->fetchAll();
        if (!count($users)) {
            $msg = "There aren't any users for the mean time !";
        }
        // die(var_dump($users));
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <a href="addUser.php" class="btn btn-outline-primary text-uppercase">Add a user +</a>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($users as $user) { ?>
                                    <tr>
                                        ...
                                        <td><?= $user['email'] ?></td>
                                        <td>
                                            <a href="editUser.php?id=<?= $user['id'] ?>" class="btn btn-sm btn-outline-secondary">Edit</a>
                                            <a href="users.php?id=<?= $user['id'] ?>" class="btn btn-sm btn-outline-danger">Delete</a>
                                        </td>
                                    </tr>
                                <?php $i++;
                                }; ?>
