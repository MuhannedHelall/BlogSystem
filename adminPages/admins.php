...
if ($_SESSION['user']['role_id'] == '1') {
    ...
    $id = 0;
    try {
        $stmt = $conn->prepare("SELECT * FROM users WHERE role_id = '1' AND visibility = '1';");
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $admins = $stmt->fetchAll();
        if (!count($admins)) {
            $msg = "There aren't any admins in the system !";
        }
        // die(var_dump($admins));
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <a href="addAdmin.php" class="btn btn-outline-primary text-uppercase">Add an admin +</a>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($admins as $admin) { ?>
                                    <tr>
                                        ...
                                        <td><?= $admin['email'] ?></td>
                                        <td>
                                            <a href="editUser.php?id=<?= $admin['id'] ?>" class="btn btn-sm btn-outline-secondary">Edit</a>
                                            <a href="admins.php?id=<?= $admin['id'] ?>" class="btn btn-sm btn-outline-danger">Delete</a>
                                        </td>
                                    </tr>
                                <?php $i++;
                                }; ?>
