...
if ($_SESSION['user']['role_id'] == '1') {
    ...
    try {
        ...
        FROM posts JOIN categories ON posts.category_id = categories.id WHERE visibility = '1';"
        );
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $posts = $stmt->fetchAll();
        if (!count($posts)) {
            $msg = "There aren't any posts on the system !";
        }
        // die(var_dump($posts));
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        if (isset($_GET['id'])) {
            ...
            if ($id) {
                try {
                    $sql = "UPDATE posts SET visibility = 0 WHERE id = $id";
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <a href="addPost.php" class="btn btn-outline-primary">Add a post +</a>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div class="album py-5">
                        <div class="container">
                            <h2 class="text-center text-uppercase"><?= $msg ?></h2>
                            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                                <?php foreach ($posts as $post) {
                                    if (!$post['visibility']) continue;
                                ?>
                                    <div class="col">
                                        <div class="card shadow-sm">
                                            <img src="<?= $post['image'] ?>" class="card-img-top" width="100%" height="225" />
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div class="btn-group">
                                                        ...
                                                        <a href="editPost.php?id=<?= $post['id'] ?>" class="btn btn-sm btn-outline-secondary">Edit</a>
                                                        <a href="Posts.php?id=<?= $post['id'] ?>" class="btn btn-sm btn-outline-danger">Delete</a>
                                                    </div>
