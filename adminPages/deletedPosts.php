...
if ($_SESSION['user']['role_id'] == '1') {
    ...
    try {
        ...
        FROM posts JOIN categories ON posts.category_id = categories.id WHERE posts.visibility = 0;"
        );
        $stmt->execute();
        $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $posts = $stmt->fetchAll();
        if (!count($posts)) {
            $msg = "There aren't any deleted post for the mean time !";
        }
        // die(var_dump($posts));
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET') {
        if (isset($_GET['id'])) {
            ...
            if ($id) {
                try {
                    $sql = "UPDATE posts SET visibility = 1 WHERE id = $id";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute();
                    header('location: deletedPosts.php');
                } catch (PDOException $e) {
                    echo $sql . "<br>" . $e->getMessage();
                }
            }
            ...
            <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
                <div class="position-sticky pt-3 sidebar-sticky">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="deletedPosts.php">
                                        ...
                                        <div class="card shadow-sm">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div class="btn-group">
                                                        <a href="deletedPosts.php?id=<?= $post['id'] ?>" class="btn btn-sm btn-outline-secondary">Retrive</a>
                                                    </div>
